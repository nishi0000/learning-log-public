...
- 「HTTP APIだから、RequestContext.Http.Method で取るんだよね？」と思って書いた
...
- 「実行時に入る/入らない」以前に、**型に存在しないプロパティはコンパイルで落ちる**
    
- `APIGatewayProxyRequest` は AWS が用意した「リクエストを入れる箱」で、箱に無いものは見れない
