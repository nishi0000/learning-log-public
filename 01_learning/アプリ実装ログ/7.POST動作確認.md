...
- `/todos` に対して POST が API Gateway → Lambda に届くことを確認する
    
- `request.Body`（JSON）を受け取れることを確認する
    
- JSON不正／Bodyなしのケースで 400 を返すことを確認する
...
- POSTで `request.Body` を取得できることを CloudWatchログで確認済み
...
- `{API_BASE_URL}/prod/todos` で以下が成立
